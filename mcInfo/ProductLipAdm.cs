...
{
    /// <summary>
    /// form product bibir (Admin)
    /// </summary>
    /// <remarks>memiliki fungsi untuk yang berbeda dari user yaitu dengan beberapa fitur CRUD</remarks>
    public partial class ProductLipAdm : Form
    {
        public int idbibir;//deklarasi integer untuk menyimpan nilai primary key tble produk_bibir
        ...
        {
            idbibir = int.Parse(dgv.CurrentRow.Cells[0].Value.ToString()); //set value ke table sesuai cell datagridview;
        ...
        public void dgview()
        {
            ...
            this.dgv.Columns["id_bibir"].Visible = false; //tampilan pada datagridview kolum idbibir disembunyikan
        ...
        {
            idbibir = 0;//set var idbibirdari awal
        ...
        private void ProductLipAdm_Load(object sender, EventArgs e)
        {
            dgview(); //memanggil method dgview()
        ...
        private void btnsave_Click(object sender, EventArgs e)
        {
            ...
            {
                //deklarasi variable data adapter untuk memanggil  database yg digunakan untuk mengisi dataset
                SqlDataAdapter sda = new SqlDataAdapter("Select count (*) from produk_bibir where nama = '" + txnm + "'", koneksi);
                ...
                else
                {
                    ...
                    //deklarasi var
                    string str = "insert into produk_bibir (nama, harga, kategori, berat, stok) values(@nm, @harga, @kategori, @berat, @stok)";
        ...
        private void btnupdate_Click(object sender, EventArgs e)
        {
            ...
            if (dialogResult == DialogResult.Yes) // kondisi jika pengguna tekan yes
            {
                ...
                //deklarasi string 
                string str = "update produk_bibir set nama = @nm, harga = @harga, kategori = @kategori, berat = @berat, stok = @stok where id_bibir=@id";

                SqlCommand cmd = new SqlCommand(str, koneksi); //membuat sql query yg berisikan var str 
                cmd.CommandType = CommandType.Text; //menetapkan nilai command
                cmd.Parameters.Add(new SqlParameter("nm", txnm)); //menambahkan objek dan menetapkan nm dg var txnm
                cmd.Parameters.Add(new SqlParameter("id", idbibir));//menambahkan objek dan menetapkan id dg var idmata
                cmd.Parameters.Add(new SqlParameter("harga", txharga));//menambahkan objek dan menetapkan harga dg var txharga
        ...
        private void btndelete_Click(object sender, EventArgs e)
        {
            ...
            if (idbibir == 0)//apabila data blm dipilih
            {
                //menampilkan pesan
            ...
            {
                koneksi.Open(); //buka koneksi
                string str = "DELETE FROM produk_bibir WHERE id_bibir = @id";//deklarasi variable 
